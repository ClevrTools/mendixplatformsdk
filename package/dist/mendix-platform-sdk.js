...
class MendixSdkClient {
  ...
  constructor(
    ...
    if (username && apikey) {
      ...
      throw new Error(
        "Incomplete credentials. Please pass username and apikey, or pass username, password and openid."
      );
    }
    ...
    this._modelSdkClient =
      mendixmodelsdk_1.Model.createSdkClient(connectionConfig);
    this._platformSdkClient = new PlatformSdkClient(
      this,
      username,
      apikey,
      projectsApiEndpoint
        ? projectsApiEndpoint
        : MendixSdkClient.DEFAULT_PROJECTSAPI_ENDPOINT
    );
    this._options = options
      ? options
      : { pollDelay: MendixSdkClient.DEFAULT_POLL_DELAY };
  }
  platform() {
...
class PlatformSdkClient {
  ...
  createNewApp(projectName, projectSummary, templateUUID) {
    console.log(
      `Creating new project with name ${projectName} for user ${this._username}...`
    );
    const contents = this._createRequestContent(
      PlatformSdkClient.createNewAppXml,
      {
    ...
    return apiClient(contents)
      .then((response) => {
        ...
        console.log(
          `Project creation for user ${this._username} underway with job id: ${jobId}...`
        );
        return this._awaitJobResult(jobId);
      })
      .then((jobResult) => {
        console.log(
          `Project created successfully for user ${this._username} with id ${jobResult.result}`
        );
  ...
  deleteApp(appId) {
    ...
    const contents = this._createRequestContent(
      PlatformSdkClient.deleteAppXml,
      {
        User: this._username,
        ApiKey: this._apikey,
        ProjectID: appId
    ...
    const apiClient = rest_1
      .wrap(this._createHttpErrorCodeInterceptor("Failed to delete app"))
      .wrap(pathPrefix, {
  ...
  createOnlineWorkingCopy(project, revision) {
    console.log(
      `Creating new online working copy for project ${project.id()} : ${project.name()}`
    );
    const request = this._createRequestContent(
      PlatformSdkClient.createOnlineWorkingCopyXml,
      {
    ...
    const apiClient = rest_1
      .wrap(
        this._createHttpErrorCodeInterceptor(
          "Failed to create online working copy"
        )
      )
      .wrap(this._parseResult())
      .wrap(pathPrefix, { prefix: this._projectsApiEndpoint });
    return apiClient(request)
      ...
      .then((jobResult) => {
        ...
        console.log(
          `Successfully created new online working copy ${wcId} for project ${project.id()} : ${project.name()}`
        );
        return when_1.promise((resolve, reject) => {
          this._client.model().openWorkingCopy(
            wcId,
            (model) => {
              console.log(
                `Successfully opened new online working copy ${wcId} for project ${project.id()} : ${project.name()}`
              );
              const rev = revision
                ? revision
                : new Revision(-1, new Branch(project, null));
              const workingCopy = new OnlineWorkingCopy(
                this._client,
                wcId,
                rev,
                model
            ...
            (error) => {
              console.error(
                `Failed to open new online working copy ${wcId} for project ${project.id()}: ${project.name()}:`
              );
              reject(error);
            }
          );
  ...
  commitToTeamServer(workingCopy, branchName = null, baseRevision = -1) {
    if (workingCopy === null || workingCopy.project() === null) {
      return when_1.reject(
        "Working copy is empty or does not contain referral to project"
      );
    } else if (baseRevision < -1) {
      return when_1.reject(`Invalid base revision ${baseRevision}`);
    }
    const teamServerInfo = `team server project ${workingCopy
      .project()
      .id()} branch ${branchName} base revision ${baseRevision}`;
    console.log(
      `Committing changes in online working copy ${workingCopy.id()} to ${teamServerInfo}`
    );
    const request = this._createRequestContent(
      PlatformSdkClient.commitWorkingCopyChangesXml,
      {
        Username: this._username,
        ApiKey: this._apikey,
    ...
    const apiClient = rest_1
      .wrap(
        this._createHttpErrorCodeInterceptor("Failed to commit to team server")
      )
      .wrap(this._parseResult())
      .wrap(pathPrefix, { prefix: this._projectsApiEndpoint });
    return apiClient(request)
      ...
      .then((jobResult) => {
        return when_1.promise((resolve, reject) => {
          ...
          if (num === null) {
            reject(
              `Failed to commit changes to team server: revision ${num} on branch ${branchName}. Reason: returned job id is not a number.`
            );
          } else {
            console.log(
              `Successfully committed changes to team server: revision ${num} on branch ${branchName}`
            );
  ...
  _awaitJobResult(jobId) {
    return when_1.promise((resolve, reject) => {
      setTimeout(() => {
        const request = this._createRequestContent(
          PlatformSdkClient.retrieveJobStatusXml,
          { JobId: jobId }
        );
        const client = rest_1
          .wrap(
            this._createHttpErrorCodeInterceptor(
              "Error when retrieving job status"
            )
          )
    ...
    return interceptor({
      success: (response) => {
        if (lodash_1.isEmpty(response.entity)) {
          ...
          return this._parseAndQuery(response.entity, "$..Result[0]").then(
            (result) => {
        ...
        if (err) {
          ...
          if (lodash_1.isEmpty(parseResult)) {
            const error = new Error(
              `Query ${query} on ${parseResult} does not give any result`
            );
            reject(error);
          } else {
            resolve(parseResult);
  ...
  _createHttpErrorCodeInterceptor(errorMessage) {
    const responseAction = (response) => {
      return when_1.promise((resolve, reject) => {
        if (
          lodash_1.isEmpty(response.status) ||
          lodash_1.isEmpty(response.entity)
        ) {
          reject(
            `Unexpected HTTP response code: ${response.status.code} ${response.raw.response.statusMessage}.`
          );
        } else if (
          response.status.code ===
          PlatformSdkClient.HTTP_STATUS_OK_RESPONSE_CODE
        ) {
          resolve(response);
        } else if (
          response.status.code ===
          PlatformSdkClient.HTTP_STATUS_WS_ERROR_RESPONSE_CODE
        ) {
          this._parseAndQuery(response.entity, "$..faultstring[0]").done(
            (cause) =>
          ...
          const retryMessage =
            "Please retry after a few minutes. If the problem persists, please consult https://mxforum.mendix.com";
          reject(
            `Unexpected HTTP response code: ${response.status.code} ${response.raw.response.statusMessage}. ${retryMessage}`
          );
        }
      });
    };
...
PlatformSdkClient.createNewAppXml =
  PlatformSdkClient._templatePath("CreateNewApp.xml");
PlatformSdkClient.deleteAppXml =
  PlatformSdkClient._templatePath("DeleteApp.xml");
PlatformSdkClient.createOnlineWorkingCopyXml = PlatformSdkClient._templatePath(
  "CreateOnlineWorkingCopy.xml"
);
PlatformSdkClient.commitWorkingCopyChangesXml = PlatformSdkClient._templatePath(
  "CommitWorkingCopyChanges.xml"
);
PlatformSdkClient.retrieveJobStatusXml = PlatformSdkClient._templatePath(
  "RetrieveJobStatus.xml"
);
  ...
  commit(branchName, baseRevision) {
    return when_1
      ...
      .then(() => {
        return this._client
          .platform()
          .commitToTeamServer(this, branchName, baseRevision);
      });
  }
}
