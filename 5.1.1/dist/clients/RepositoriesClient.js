...
class RepositoriesClient {
  get restConnector() {
    return RestConnector_1.RestConnector.create(
      (0, config_1.getPlatformConfig)().repositoryServiceEndpoint,
      {
        type: "pat",
  ...
  async createTemporaryWorkingCopy(appId, branchName, options) {
    logger_1.logger.info(
      `Creating temporary working copy for branch '${branchName}'...`
    );
    const workingCopyId = await (0, retry_1.retry)(
      async () => {
        const response = await this.restConnector.awaitTask(
          "/v1/temporary-working-copies/",
          {
            projectId: appId,
            branchName,
      ...
      },
      {
        ...
        retryDelayMs: 200,
        shouldRetry: (error) =>
          !["RS400", "RS401", "RS403", "RS404"].includes(error.errorCode)
      }
    );
    logger_1.logger.info(
      `Successfully created temporary working copy with id '${workingCopyId}' based on branch '${branchName}'`
    );
   ...
   */
  async commitTemporaryWorkingCopy(
    appId,
    workingCopyId,
    targetBranchName,
    options
    ...
    const branchName =
      targetBranchName ?? (await this.getWorkingCopyBaseBranch(workingCopyId));
    logger_1.logger.info(
      `Committing temporary working copy '${workingCopyId}' to branch '${branchName}'...`
    );
    await (0, retry_1.retry)(
      async () => {
        ...
        await this.restConnector.awaitTask(
          `/v1/temporary-working-copies/${workingCopyId}/commit`,
          body
        );
      },
      {
    ...
    logger_1.logger.info(
      `Successfully committed the working copy with id '${workingCopyId}' to branch '${branchName}'`
    );
  }
  /**
  ...
  async getRepositoryInfo(appId) {
    const response = await this.restConnector.get(
      `/v1/repositories/${appId}/info`
    );
  ...
  async getBranches(appId, options) {
    const response = await this.restConnector.get(
      `/v1/repositories/${appId}/branches`,
      options
    );
  ...
  async getBranch(appId, branchName) {
    const response = await this.restConnector.get(
      `/v1/repositories/${appId}/branches/${branchName}`
    );
    return response.data;
  }
  /**
  ...
  async getBranchCommits(appId, branchName, options) {
    const response = await this.restConnector.get(
      `/v1/repositories/${appId}/branches/${branchName}/commits`,
      options
    );
    return response.data;
  }
  async getWorkingCopyBaseBranch(workingCopyId) {
    const wcBaseBranch = (
      await (0, modelSdkHelpers_1.getModelSdkClient)().loadWorkingCopyMetaData(
        workingCopyId
      )
    ).metaData.teamServerBaseBranch;
    if (wcBaseBranch) {
      return wcBaseBranch;
    }
    throw new Error(
      "Cannot retrieve branch from working copy, please specify the target branch name."
    );
  }
}
